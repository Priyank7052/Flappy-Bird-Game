...
if "bird_y" not in st.session_state:
    ...
    st.session_state.pipes = [(WIDTH, random.randint(5, HEIGHT - PIPE_GAP - 5))]
...
def update():
    ...
    for x, h in st.session_state.pipes:
        if x > 0:
            ...
            new_pipes.append((WIDTH, random.randint(5, HEIGHT - PIPE_GAP - 5)))
...
def draw():
    ...
    for y in range(HEIGHT):
        ...
        for x in range(WIDTH):
            if x == 3 and y == st.session_state.bird_y:
                row += "🐤"
...
st.title("🐦 Flappy Bird (Text Version)")
